...
(function(orig){
  ...
  if(btnAddDate){
    btnAddDate.addEventListener('click', async ()=>{
      const d = (addDateEl && addDateEl.value)||''
      if(!/^\d{4}-\d{2}-\d{2}$/.test(d)){ statusEl.textContent='日付を選択してください'; return }
      try{
        const r = await fetch('/api/shared/session/'+token+'/allowed_dates', {method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({dates:[d]})})
        if(!r.ok){ const t=await r.text(); throw new Error('append_failed:'+t) }
        const jsn = await r.json()
        allowed = jsn.allowed_dates||allowed
        renderAllowed()
        statusEl.textContent = '候補日を追加しました'
      }catch(e){
        console.error(e); statusEl.textContent = '候補日の追加に失敗しました: '+(e.message||'')
      }
    })
  }
