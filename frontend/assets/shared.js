...
btnCreate.addEventListener('click', async ()=>{
    ...
    try{
      const r = await fetch(new URL('/api/shared/session', window.location.origin), {
        method:'POST', headers:{'content-type':'application/json'},
        body: JSON.stringify({ dates:picked, mode, title:titleEl.value||'' })
      })
      if(!r.ok){ let txt=''; try{txt=await r.text()}catch(_){}; throw new Error('create_failed:'+txt) }
      const data = await r.json()
      shareUrlEl.innerHTML = `共有URL: <a href="${data.url}" target="_blank" rel="noopener noreferrer">${data.url}</a>`
    }catch(e){
      console.error(e);
      shareUrlEl.textContent='リンクの発行に失敗しました: '+(e.message||'')
    }
  })
})()
