...
export default function Personal() {
  ...
  const onSubmit = async (e) => {
    ...
    for (const d of ds) {
      await addPersonal({ user: user || '(guest)', title, memo, date: d, time_slot: timeSlot });
    }
      ...
      <div className="row" style={{alignItems:'flex-start'}}>
        <div style={{flex:'0 0 340px'}}>
          ...
          <div className="row" style={{marginTop:8}}>
            <button className="button ghost" onClick={()=>{ setRangeStart(todayIso); setRangeEnd(todayIso); }}>今日に戻る</button>
            <button className="button ghost" onClick={()=>{ setRangeStart(rangeStart); setRangeEnd(null); }}>単日指定に戻す</button>
          </div>
        </div>
          ...
          <div className="card" style={{marginTop:0}}>
            <div className="row" style={{marginBottom:10}}>
              <div style={{minWidth:220}}>
                ...
                <input className="input" readOnly value={rangeEnd ? `${rangeStart} 〜 ${rangeEnd}` : rangeStart} />
              </div>
            ...
            <div className="row">
              <button className="button" type="submit">
                {rangeEnd && rangeStart !== rangeEnd ? '範囲分を一括追加' : '追加'}
              </button>
            </div>
          </div>
        </form>
