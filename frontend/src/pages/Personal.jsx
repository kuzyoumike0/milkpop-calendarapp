...
export default function Personal(){
  ...
  const add = async (e)=>{
    ...
    for(const d of dates){
      await fetch('/api/personal',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title, memo, date:d, time_slot:timeSlot})})
    }
    setTitle(''); setMemo(''); setTimeSlot('全日'); fetchData()
  }
        ...
        <form onSubmit={add} style={{flex:1}}>
          <div className="card" style={{marginTop:0}}>
            ...
            <div style={{marginBottom:10}}>
              ...
              <textarea className="input" rows={3} placeholder="補足を入力" value={memo} onChange={e=>setMemo(e.target.value)} style={{width:'100%'}}/>
            </div>
            <div className="row">
              <input className="input" readOnly value={range.end? `${range.start} 〜 ${range.end}`: range.start} style={{flex:1}}/>
              <button className="button" type="submit">{range.end && range.start!==range.end ? '範囲一括追加' : '追加'}</button>
            </div>
          </div>
        </form>
      </div>
