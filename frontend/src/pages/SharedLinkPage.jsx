...
export default function SharedLinkPage() {
  ...
  const [timeType, setTimeType] = useState("all"); // all=全日昼夜, custom=時間指定
  ...
  const addEvent = (e) => {
    ...
    setEvents(prev => ({
      ...prev,
      [selectedDate]: [...(prev[selectedDate] || []), eventStr]
    }));
    setTitle("");
  };
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', sans-serif", background: "#000", minHeight: "100vh", color: "#fff" }}>
      <header style={{ background: "#004CA0", color: "#FDB9C8", padding: "1rem", textAlign: "center", fontSize: "1.8rem", fontWeight: "bold" }}>
        MilkpopCalendar Shared
      </header>

      <main style={{ maxWidth: 900, margin: "2rem auto", padding: "1rem", background: "#111", borderRadius: "12px", boxShadow: "0 2px 12px rgba(0,0,0,0.5)" }}>
        <h2 style={{ textAlign: "center", color: "#FDB9C8" }}>共有スケジュール</h2>

        <form onSubmit={addEvent} style={{ display: "flex", flexDirection: "column", gap: "0.5rem", marginBottom: "1rem" }}>
                ...
                <tr key={wi}>
                  {week.map((day, di)=>(
                    <td key={di} onClick={()=>setSelectedDate(day.str)} style={{ cursor:"pointer", padding:".75rem", textAlign:"center", borderRadius:"8px",
                      background: day.str===selectedDate ? "#004CA0" : day.isToday ? "#FDB9C8" : day.inMonth ? "#111" : "#333",
                      color: day.str===selectedDate ? "#fff" : day.isToday ? "#000" : "#fff",
        ...
        <div style={{ textAlign: "center" }}>
          <button onClick={generateShareLink} style={{ padding: "0.75rem 1.5rem", background: "#004CA0", color: "#FDB9C8", border: "none", borderRadius: "8px", fontWeight: "bold" }}>
            新しい共有リンクを発行
          </button>
        </div>
      </main>
