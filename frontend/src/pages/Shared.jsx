...
export default function Shared(){
  ...
  const t=new Date(); const [y,setY]=useState(t.getFullYear()); const [m,setM]=useState(t.getMonth()+1)
  const today = new Date().toISOString().slice(0,10)
  const [range,setRange]=useState({start:today,end:today})

  const fetchData = async()=>{ const r=await fetch('/api/shared'); setItems(await r.json()) }
  useEffect(()=>{ fetchData() }, [])
  ...
  const add = async (e)=>{
    ...
    for(const d of dates){
      const r = await fetch('/api/shared',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title, date:d, time_slot:timeSlot})})
      const out = await r.json()
      url = out.item?.share_url || url
    }
    setLastURL(url); setTitle(''); setTimeSlot('全日'); fetchData()
  }
  const del = async (id)=>{ await fetch('/api/shared/'+id,{method:'DELETE'}); fetchData() }

  const prev=()=>{ const nm=m-1; if(nm<1){setM(12); setY(y-1)} else setM(nm) }
  const next=()=>{ const nm=m+1; if(nm>12){setM(1); setY(y+1)} else setM(nm) }
      ...
      <div style={{display:'grid',gridTemplateRows:'auto auto',placeItems:'center',gap:4}}>
        ...
        <div style={{display:'inline-flex',gap:4}}>
          {list.slice(0,3).map((ev,i)=>(
            <span key={ev.id||i} className="badge"
              title={`${ev.title} [${ev.time_slot}]`}
              style={{background: ev.time_slot==='全日' ? '#22d3ee' : ev.time_slot==='昼' ? '#34d399' : '#f472b6'}} />
          ))}
    ...
    <section className="card">
      ...
      <div className="row" style={{alignItems:'flex-start'}}>
        <div style={{flex:'0 0 420px'}}>
          <UniversalCalendar mode="range" year={y} month={m} range={range}
            onRangeChange={setRange} onPrev={prev} onNext={next} renderDay={renderDay} />
          <div className="row" style={{marginTop:8}}>
            <button className="button ghost" onClick={()=> setRange({start:today,end:today})}>今日</button>
            <button className="button ghost" onClick={()=> setRange(r=>({start:r.start,end:null}))}>単日</button>
          </div>
        </div>

        <form onSubmit={add} style={{flex:1}}>
          <div className="card" style={{marginTop:0}}>
            <div className="row" style={{marginBottom:10}}>
              <input className="input" style={{flex:1}} placeholder="タイトル" value={title} onChange={e=>setTitle(e.target.value)} required />
            ...
            {lastURL && (
              <div className="card" style={{marginTop:8}}>
                ...
                <div className="row">
                  <input className="input" readOnly value={lastURL} style={{flex:1}}/>
                  <button className="button" type="button" onClick={async()=>{
                    try{ await navigator.clipboard.writeText(lastURL); alert('コピーしました') }catch{ prompt('コピーしてください', lastURL) }
                  }}>コピー</button>
                </div>
              </div>
            )}
      ...
      <div style={{marginTop:8}}>
        <ul className="list">
          {Object.entries(map).sort(([a],[b])=>a.localeCompare(b)).map(([d,list])=> (
            <li key={d} className="item" style={{flexDirection:'column',alignItems:'flex-start'}}>
              <div><strong>{d}</strong></div>
              {list.map(ev=> (
                <div key={ev.id} className="item" style={{border:'none',paddingLeft:0}}>
