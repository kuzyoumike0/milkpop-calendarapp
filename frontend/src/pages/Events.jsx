...
export default function Events(){
  const [items,setItems]=useState([]); const [title,setTitle]=useState(''); const [date,setDate]=useState('')
  const fetchData = async()=>{ const r=await fetch('/api/events'); if(r.ok) setItems(await r.json()) }
  useEffect(()=>{ fetchData() }, [])
