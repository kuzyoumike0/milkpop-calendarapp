...
export default function SharedCalendar() {
  ...
  const fetchEvents = () => {
    ...
    axios.get(`/api/shared?date=${formattedDate}`).then(res => setEvents(res.data));
  };
  ...
  const addEvent = () => {
    const formattedDate = dayjs(date).format("YYYY-MM-DD");
    axios.post("/api/shared", { title, user, time_slot: timeSlot, date: formattedDate })
      .then(() => {
  ...
  const deleteEvent = (id) => {
    axios.delete(`/api/shared/${id}`).then(() => fetchEvents());
  };
