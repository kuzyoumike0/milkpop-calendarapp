...
// 共有リンクから予定一覧を取得（開始時刻昇順でソート）
app.get("/api/share/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query(
      `SELECT * FROM schedules WHERE share_id=$1 ORDER BY start_time ASC`,
      [id]
    );
...
app.post("/api/share/:id/add", async (req, res) => {
  const { id } = req.params;
  const { username, start_time, end_time, title } = req.body;
  try {
    await pool.query(
      `INSERT INTO schedules (date, username, start_time, end_time, title, share_id)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [new Date().toISOString().split("T")[0], username, start_time, end_time, title, id]
    );
