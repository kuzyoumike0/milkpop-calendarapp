...
export default function UniversalCalendar({mode='single',year,month,value,onChange,range,onRangeChange,onPrev,onNext,renderDay}){
  ...
  const click=(d)=>{if(!d)return;const v=iso(year,month,d);if(mode==='single'&&onChange)onChange(v);if(mode==='range'&&onRangeChange){const s=range?.start||null,e=range?.end||null;if(!s||(s&&e))onRangeChange({start:v,end:null});else if(v<s)onRangeChange({start:v,end:null});else onRangeChange({start:s,end:v})}}
