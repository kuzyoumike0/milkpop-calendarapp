...
export default function UniversalCalendar({
  ...
  onPrev, onNext, renderDay, onDayClick
}){
  ...
  const weeks = useMemo(()=>{
    ...
    const out=[]; for(let i=0;i<cells.length;i+=7) out.push(cells.slice(i,i+7))
    return out
  },[year,month])
  ...
  return (
    <div className="card" style={{padding:16}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:6}}>
        ...
        {weeks.flat().map((d,i)=>{
          const blank = d===null
          const v = !blank ? iso(year,month,d) : null
          const selected = (mode==='single' && value===v) ||
                           (mode==='range' && v && range && (range.end ? (v>=range.start && v<=range.end) : v===range.start))
