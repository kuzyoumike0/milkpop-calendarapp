...
function Calendar() {
  ...
  const addShared = async () => {
    if (!newShared.date || !newShared.title) return alert("日付とタイトルは必須です");
  ...
  const addPersonal = async () => {
    if (!newPersonal.shared_id) return alert("紐づける共有予定を選択してください");
  ...
  return (
    <div>
      ...
      <input
        type="date"
        value={newShared.date}
        onChange={(e) => setNewShared({ ...newShared, date: e.target.value })}
      />
      <select
        value={newShared.period}
        onChange={(e) => setNewShared({ ...newShared, period: e.target.value })}
      >
      ...
      <input
        type="text"
        placeholder="タイトル"
        value={newShared.title}
        onChange={(e) => setNewShared({ ...newShared, title: e.target.value })}
      />
      ...
      <ul>
        {personal.map((p) => (
          <li key={p.id}>
            {p.date} - {p.period} : {p.title} ({p.note})
          </li>
        ))}
      </ul>
      ...
      <h3>個人予定を追加（メモ）</h3>
      <select
        value={newPersonal.shared_id}
        onChange={(e) => setNewPersonal({ ...newPersonal, shared_id: e.target.value })}
      >
