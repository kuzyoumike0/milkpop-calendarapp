...
export default function CalendarList({ refresh }) {
  ...
  return (
    <div className="mt-6">
      ...
      {personalEvents.length === 0 ? <p>イベントがありません</p> :
      ...
      {sharedEvents.length === 0 ? <p>共有イベントはありません</p> :
        <ul className="space-y-2">
          {sharedEvents.map(ev => (
            <li key={ev.id} className="border p-2 rounded shadow-sm hover:bg-pink-50 transition">
              <span className="font-semibold">{ev.date} [{ev.time_slot}]</span>: {ev.title} (作成者: {ev.create_by})
            </li>
          ))}
        </ul>
      }
