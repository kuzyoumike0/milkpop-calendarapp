...
import SharePublicUser from './pages/SharePublicUser.jsx'
...
export default function App(){
  ...
  if(path.startsWith('/p/')){return <SharePublicUser token={path.split('/p/')[1]||''} />}
  ...
  useEffect(()=>{(async()=>{
    try{
      ...
      if (window.location.pathname === '/') {
        if (d.loggedIn) goto(`/u/${d.slug}`)
        else window.location.replace('/auth/guest')
      }
