...
async function migrate(){
  ...
  // ensure share_token exists on legacy users tables
  ...
  await safeExec(`ALTER TABLE shared_events RENAME COLUMN create_by TO user_id;`) // 互換：create_by を user_id に
...
// users helpers
async function upsertUserByEmail({email,name,provider}){
  if(!hasPG){ // memory
    const id = Math.floor(Math.random()*1e9)
    const share_token = crypto.randomBytes(16).toString('hex')
    return { id, email, name, provider, share_token }
  }
  ...
  if(sel.rowCount>0){
    ...
    if(!u.share_token){
      ...
      const upd=await pool.query('update users set share_token=$1 where id=$2 returning *',[tok,u.id])
      return upd.rows[0]
    }
    return u
  } else {
    const tok=crypto.randomBytes(16).toString('hex')
    const ins=await pool.query('insert into users(email,name,provider,share_token) values($1,$2,$3,$4) returning *',[email,name,provider,tok])
    return ins.rows[0]
  }
}
  ...
  passport.use(new GoogleStrategy({ clientID:GID, clientSecret:GSECRET, callbackURL:GCALLBACK },
    async (accessToken,refreshToken,profile,done)=>{
      try{
        ...
        const user=await upsertUserByEmail({email,name,provider:'google'})
  ...
  passport.use(new TwitterStrategy({ consumerKey:TW_KEY, consumerSecret:TW_SECRET, callbackURL:TW_CALLBACK, includeEmail:true },
    async (token,tokenSecret,profile,done)=>{
      try{
        ...
        const user=await upsertUserByEmail({email,name,provider:'twitter'})
...
app.get('/auth/guest', async (req,res)=>{
  // assign or reuse a single guest record per session (id 0 if DBなし)
  if(!hasPG){ req.session.user={...makeGuest(), user_id:0, provider:'guest'}; return res.redirect(`/u/0`) }
  // create / find shared guest by session email
  const email = `guest_${crypto.randomBytes(4).toString('hex')}@guest.local`
  const user = await upsertUserByEmail({email,name:'Guest',provider:'guest'})
...
async function addShared(uid,{title,date,time_slot}){ if(hasPG){ const {rows}=await pool.query('insert into shared_events(user_id,date,time_slot,title) values($1,$2,$3,$4) returning *',[uid,date,time_slot,title]); return rows[0]} const item={id:String(Date.now())+Math.random().toString(36).slice(2,8),title,date,time_slot,user_id:uid,created_at:new Date().toISOString()}; mem.shared[uid]=(mem.shared[uid]||[]).concat([item]); return item }
...
async function getShareItemsByToken(tok){ if(hasPG){ const u=await pool.query('select id from users where share_token=$1',[tok]); if(u.rowCount===0) return []; const uid=u.rows[0].id; const {rows}=await pool.query('select * from shared_events where user_id=$1 order by date asc,id asc',[uid]); return rows } for(const uid of Object.keys(mem.shared)){ /* not used in mem */ } return [] }
...
app.post('/api/personal', requireAuth, async (req,res)=>{
  ...
  if(publish_to_shared){ await addShared(uid,{title,date,time_slot}) }
...
app.post('/api/shared', requireAuth, async (req,res)=>{ const uid=uidFromReq(req); const { title, date, time_slot } = req.body; const item=await addShared(uid,{title,date,time_slot}); res.json({success:true,item}) })
