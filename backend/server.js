...
// --- runtime migration: add memo column for shared_events ---
if (hasPG && typeof pool !== 'undefined') {
  (async ()=>{
    try{
      await pool.query("ALTER TABLE IF EXISTS shared_events ADD COLUMN IF NOT EXISTS memo text");
    }catch(e){ console.warn('[migrate] add memo failed (safe to ignore if no DB):', e.message) }
  })();
}
app.use(cors())
...
app.post('/api/shared/session/:token/register', async (req,res)=>{
  const { date, time_slot, member_name='' } = req.body||{}
  ...
  const ins = await pool.query('INSERT INTO shared_events(user_id,date,time_slot,title,member_name,created_by,token, memo) VALUES ($1,$2,$3,$4,$5,$6,$7, $X) RETURNING *',
    [owner, date, time_slot, '', member_name, owner, req.params.token])
...
app.get('/api/shared/session/:token/events', async (req,res)=>{
  try{
    ...
    const r = await pool.query('SELECT id, date, time_slot, member_name, memo FROM shared_events WHERE token=$1 ORDER BY date, member_name', [req.params.token])
    res.json(r.rows||[])
  }catch(e){
...
app.delete('/api/shared/session/:token/events/:id', async (req,res)=>{
  try{
    if(!hasPG) return res.status(501).json({error:'no_db'})
    const { member_name='' } = req.body||{}
    if(!member_name) return res.status(400).json({error:'member_name_required'})
    const r = await pool.query('DELETE FROM shared_events WHERE id=$1 AND token=$2 AND member_name=$3', [req.params.id, req.params.token, member_name])
...
app.post('/api/shared/session/:token/allowed_dates', async (req,res)=>{
  try{
    if(!hasPG) return res.status(501).json({error:'no_db'})
    const { dates=[] } = req.body||{}
    if(!Array.isArray(dates) || dates.length===0) return res.status(400).json({error:'dates_required'})
    // Get current
    const cur = await pool.query('SELECT allowed_dates FROM shared_sessions WHERE token=$1', [req.params.token])
    if(cur.rowCount===0) return res.status(404).json({error:'session_not_found'})
    const now = new Set((cur.rows[0].allowed_dates||[]).map(String))
    for(const d of dates){ if(d && /^\d{4}-\d{2}-\d{2}$/.test(d)) now.add(String(d)) }
    const merged = Array.from(now).sort()
    await pool.query('UPDATE shared_sessions SET allowed_dates=$2 WHERE token=$1', [req.params.token, JSON.stringify(merged)])
    res.json({ok:true, allowed_dates: merged})
  }catch(e){
    console.error(e); res.status(500).json({error:'append_failed'})
  }
})
...
app.put('/api/shared/session/:token/events/:id', async (req,res)=>{
  try{
    ...
    if(!member_name) return res.status(400).json({error:'member_name_required'})
    if(!time_slot) return res.status(400).json({error:'time_slot_required'})
