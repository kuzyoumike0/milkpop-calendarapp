...
async function ensureSchema(){
  try{
    if(!hasPG || !pool){ console.warn('[DB] no db configured'); return; }
    await pool.query(`
    ...
    `);
    console.log('[DB] migrations ok (shared + personal)');
  }catch(e){
...
function normalizeDatesFromBody(body){
  ...
  if(Array.isArray(body?.allowed_dates)){
    body.allowed_dates.forEach(push);
  }else if(typeof body?.allowed_dates === 'string'){
    body.allowed_dates.split(',').forEach(push);
  }
  if(Array.isArray(body?.selectedDates)){
    body.selectedDates.forEach(push);
  }
...
app.post('/api/shared/session', async (req,res)=>{
  ...
  }catch(e){ console.error(e); res.status(500).json({error:'issue_failed'}); }
});
...
app.get('/api/shared/session/:token/events', async (req,res)=>{
  try{
    if(!hasPG || !pool) return res.status(501).json({error:'no_db'});
    const { rows } = await pool.query('SELECT id, token, date, time_slot, member_name, memo, created_at FROM shared_events WHERE token=$1 ORDER BY date ASC, member_name ASC', [req.params.token]);
    res.json(rows);
...
app.post('/api/shared/session/:token/register', async (req,res)=>{
  try{
    ...
    await pool.query('INSERT INTO shared_events (token,date,time_slot,member_name,memo) VALUES ($1,$2,$3,$4,$5)', [token, date, time_slot, member_name, memo]);
...
app.put('/api/shared/session/:token/events/:id', async (req,res)=>{
  try{
    ...
    const upd = await pool.query('UPDATE shared_events SET time_slot=$1, memo=$2 WHERE id=$3 AND token=$4 AND member_name=$5', [time_slot, memo, req.params.id, req.params.token, member_name]);
...
app.get('/api/personal', async (req,res)=>{
  try{
    ...
    const { rows } = user
      ? await pool.query('SELECT id,user_name,date,time_slot,memo,created_at FROM personal_schedules WHERE user_name=$1 ORDER BY date ASC',[user])
      : await pool.query('SELECT id,user_name,date,time_slot,memo,created_at FROM personal_schedules ORDER BY date ASC');
    res.json(rows);
  }catch(e){ console.error(e); res.status(500).json({error:'list_failed'}); }
});
...
app.put('/api/personal/:id', async (req,res)=>{
  try{
    ...
    if(!user_name || !isValidTimeSlot(time_slot)) return res.status(400).json({error:'bad_request'});
    const upd = await pool.query('UPDATE personal_schedules SET time_slot=$1, memo=$2 WHERE id=$3 AND user_name=$4', [time_slot, memo, req.params.id, user_name]);
    if(upd.rowCount===0) return res.status(404).json({error:'not_found'});
    res.json({success:true});
  }catch(e){ console.error(e); res.status(500).json({error:'update_failed'}); }
});
