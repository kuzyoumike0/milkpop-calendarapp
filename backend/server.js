...
// ===== Schema ensure (define after hasPG) =====
...
app.get('/api/shared/session/:token', async (req,res)=>{
  try{
    ...
    console.error(e); res.status(500).json({error:'get_failed'});
  }
});
...
// ----- Personal schedule APIs -----
app.get('/api/personal', async (req,res)=>{
  try{
    if(!hasPG || !pool) return res.status(501).json({error:'no_db'});
    const user = (req.query.user||'').trim();
    const q = user ? 'SELECT * FROM personal_schedules WHERE user_name=$1 ORDER BY date ASC' 
...
app.put('/api/personal/:id', async (req,res)=>{
  try{
    ...
    const upd = await pool.query('UPDATE personal_schedules SET time_slot=$1, memo=$2 WHERE id=$3 AND user_name=$4', [time_slot, memo, id, user_name]);
    if(upd.rowCount===0) return res.status(404).json({error:'not_found'});
    res.json({success:true});
...
app.delete('/api/personal/:id', async (req,res)=>{
  try{
    ...
    const del = await pool.query('DELETE FROM personal_schedules WHERE id=$1 AND user_name=$2', [id, user_name]);
    if(del.rowCount===0) return res.status(404).json({error:'not_found'});
    res.json({success:true});
  }catch(e){ console.error(e); res.status(500).json({error:'delete_failed'}); }
});

// Ensure schema then start
await ensureSchema();
