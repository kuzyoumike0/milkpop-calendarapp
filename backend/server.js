...
async function migrate(){
  ...
  await safeExec(`ALTER TABLE shared_events ALTER COLUMN created_by DROP NOT NULL;`)
...
async function addShared(uid,{title,date,time_slot,member_name}){ if(hasPG){ const {rows}=await pool.query('insert into shared_events(user_id,date,time_slot,title,member_name,created_by) values($1,$2,$3,$4,$5,$6) returning *',[uid, date, time_slot, title, member_name, uid]); return rows[0]} const item={id:String(Date.now())+Math.random().toString(36).slice(2,8),title,date,time_slot,user_id:uid,member_name,created_at:new Date().toISOString()}; mem.shared[uid]=(mem.shared[uid]||[]).concat([item]); return item }
