...
app.get('/api/shared/session/:token/events', async (req,res)=>{
  try{
    ...
    const r = await pool.query('SELECT id, date, time_slot, member_name FROM shared_events WHERE token=$1 ORDER BY date, member_name', [req.params.token])
    res.json(r.rows||[])
  }catch(e){
...
app.put('/api/shared/session/:token/events/:id', async (req,res)=>{
  try{
    if(!hasPG) return res.status(501).json({error:'no_db'})
    const { member_name='', time_slot='' } = req.body||{}
    if(!member_name) return res.status(400).json({error:'member_name_required'})
    if(!time_slot) return res.status(400).json({error:'time_slot_required'})
    const r = await pool.query(
      'UPDATE shared_events SET time_slot=$1 WHERE id=$2 AND token=$3 AND member_name=$4 RETURNING id,date,time_slot,member_name',
      [time_slot, req.params.id, req.params.token, member_name]
    )
    if(r.rowCount===0) return res.status(404).json({error:'not_found_or_not_owner'})
    res.json(r.rows[0])
  }catch(e){
