...
async function ensureSchema(){
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_shared_events_token_date ON shared_events(token,date);`);
  console.log('[DB] migrations ok (shared_sessions/shared_events)');
}
ensureSchema().catch(e=>console.error('[DB] migration error', e));
const app = express()
app.use(cors())
...
app.post('/api/shared/session/:token/register', async (req,res)=>{
  try{
    ...
    const { date, time_slot, member_name, memo=null } = req.body || {}
    if(!token || !date || !time_slot || !member_name){
      return res.status(400).json({error:'bad_request'})
    }
    const ss = await pool.query('SELECT allowed_dates FROM shared_sessions WHERE token=$1', [token])
    if(ss.rowCount===0) return res.status(404).json({error:'session_not_found'})
    const allowed = ss.rows[0].allowed_dates || []
    if(!Array.isArray(allowed) || !allowed.includes(date)){
      return res.status(400).json({error:'date_not_allowed'})
    }
    const ok = (['x','am','pm','allday'].includes(time_slot)) || (/^time:\d{2}:\d{2}-\d{2}:\d{2}$/.test(time_slot))
    if(!ok) return res.status(400).json({error:'invalid_time_slot'})
  ...
  }catch(e){ console.error('insert_failed', e); res.status(500).json({error:'insert_failed'}) }
});
