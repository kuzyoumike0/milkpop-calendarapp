...
async function migrate(){
  ...
  // shared_events
  await safeExec(`create table if not exists shared_sessions(
    id serial primary key,
  ...
  await safeExec(`ALTER TABLE shared_sessions ADD COLUMN IF NOT EXISTS default_title text;`)
  await safeExec(`ALTER TABLE shared_sessions ADD COLUMN IF NOT EXISTS start_time text;`)
  await safeExec(`ALTER TABLE shared_sessions ADD COLUMN IF NOT EXISTS end_time text;`)
  ...
  await safeExec(`UPDATE shared_events SET created_by = COALESCE(created_by, user_id);`)
...
app.post('/api/shared/session', requireAuth, async (req,res)=>{
  const uid = uidFromReq(req)
  const { dates=[], mode='multi' } = req.body || {}
  if(!hasPG){ const token = crypto.randomBytes(16).toString('hex'); return res.json({ url: baseUrl(req) + '/share/s/' + token, token }) }
  if(!Array.isArray(dates) || dates.length===0) return res.status(400).json({error:'no_dates'})
  const token = crypto.randomBytes(16).toString('hex')
  await pool.query('insert into shared_sessions(owner_user_id, token, allowed_dates, mode) values($1,$2,$3,$4)', [uid, token, JSON.stringify(dates), mode])
  res.json({ url: baseUrl(req) + '/share/s/' + token, token })
})
// share session info
app.get('/api/shared/session/:token', async (req,res)=>{
  if(!hasPG) return res.status(404).json({error:'not_supported_in_memory'})
  const tk=req.params.token
  const r=await pool.query('select s.*, u.name as owner_name from shared_sessions s left join users u on u.id=s.owner_user_id where token=$1',[tk])
  if(r.rowCount===0) return res.status(404).json({error:'not_found'})
  const allowed = r.rows[0].allowed_dates || []
  ...
  res.json({ allowed_dates: allowed, mode: r.rows[0].mode||'multi', owner_name: r.rows[0].owner_name || '', default_title: r.rows[0].default_title||'', start_time: r.rows[0].start_time||'', end_time: r.rows[0].end_time||'', existing })
})
// register via session
app.post('/api/shared/session/:token/register', async (req,res)=>{
  if(!hasPG) return res.status(404).json({error:'not_supported_in_memory'})
  const tk=req.params.token
  const sess=await pool.query('select * from shared_sessions where token=$1',[tk])
  if(sess.rowCount===0) return res.status(404).json({error:'not_found'})
  const owner = sess.rows[0].owner_user_id
  const allowed = sess.rows[0].allowed_dates || []
  const { date, time_slot, title, member_name } = req.body || {}
  if(!date || !allowed.includes(date)) return res.status(400).json({error:'date_not_allowed'})
  const {rows}=await pool.query('insert into shared_events(user_id,date,time_slot,title,member_name,created_by,token) values($1,$2,$3,$4,$5,$6,$7) returning *',[owner,date,time_slot,title,member_name,owner,tk])
