...
app.get('/auth/google/login', (req,res,next)=>{
  if (!(GID && GSECRET)) return res.status(500).send('Google OAuth が未設定です。環境変数 GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET / GOOGLE_CALLBACK_URL を設定してください。');
  return passport.authenticate('google', { scope: ['profile','email'] })(req,res,next);
});
app.get('/auth/google/callback',
  (req,res,next)=>{ if(!(GID && GSECRET)) return res.redirect('/login'); next(); },
