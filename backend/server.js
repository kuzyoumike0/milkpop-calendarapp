// backend/server.js (uses "date" column instead of "event_date")
...
app.get('/api/shared/:shareId/events', async (req, res) => {
  try {
    ...
    let q = `SELECT id, date AS event_date, title, slots, memo, created_at
...
app.post('/api/shared/:shareId/events', async (req, res) => {
  try {
    ...
    slots = Array.from(new Set(slots.filter(s => ALLOW.has(String(s))))); 
    if (slots.length === 0) slots = ['全日'];

    const result = await pool.query(
      `INSERT INTO shared_events (share_id, date, title, slots, memo)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, share_id, date AS event_date, title, slots, memo, created_at`,
