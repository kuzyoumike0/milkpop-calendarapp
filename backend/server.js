...
const AUTH_MODE = process.env.AUTH_MODE || 'optional' // <- 任意ログインをデフォルト
...
// ---------------- Repository (Postgres if available, else memory) ----------------
...
if (hasPG) {
  ...
  ;(async()=>{
    await pool.query(`create table if not exists personal_events(
...
async function repo_listPersonal(email){
  if (hasPG) {
    const { rows } = await pool.query('select * from personal_events where user_email=$1 order by date asc, id asc',[email]); return rows
  } else {
    return mem.personal[email] || []
  }
}
async function repo_addPersonal(email, { title, memo='', date, time_slot }){
  if (hasPG) {
    const { rows } = await pool.query('insert into personal_events(user_email,date,time_slot,title,memo) values($1,$2,$3,$4,$5) returning *',[email,date,time_slot,title,memo]); return rows[0]
  } else {
    const item = { id: String(Date.now())+Math.random().toString(36).slice(2,8), title, memo, date, time_slot, created_at:new Date().toISOString() }
    mem.personal[email] = (mem.personal[email] || []).concat([item]); return item
  }
}
async function repo_delPersonal(email, id){
  if (hasPG) await pool.query('delete from personal_events where id=$1 and user_email=$2',[id,email])
  else mem.personal[email] = (mem.personal[email]||[]).filter(x=>x.id!==id)
}

async function repo_listShared(email){
  if (hasPG) { const { rows } = await pool.query('select * from shared_events where created_by=$1 order by date asc, id asc',[email]); return rows }
  else return mem.shared[email] || []
}
async function repo_addShared(email,{ title, date, time_slot, token, share_url }){
  if (hasPG) {
    const { rows } = await pool.query('insert into shared_events(date,time_slot,title,created_by,token) values($1,$2,$3,$4,$5) returning *',[date,time_slot,title,email,token]); const item=rows[0]; item.share_url = share_url; return item
  } else {
    const item = { id: String(Date.now())+Math.random().toString(36).slice(2,8), title, date, time_slot, created_by:email, token, created_at:new Date().toISOString(), share_url }
    mem.shared[email] = (mem.shared[email] || []).concat([item]); return item
  }
}
async function repo_delShared(email,id){
  if (hasPG) await pool.query('delete from shared_events where id=$1 and created_by=$2',[id,email])
  else mem.shared[email] = (mem.shared[email]||[]).filter(x=>x.id!==id)
}
async function repo_getSharedByToken(token){
  if (hasPG) { const { rows } = await pool.query('select * from shared_events where token=$1 limit 1',[token]); return rows[0] || null }
  else {
    for(const email of Object.keys(mem.shared)){
      const it=(mem.shared[email]||[]).find(e=>e.token===token); if(it) return it
    }
    return null
  }
}

async function repo_listEvents(email){
  if (hasPG) { const { rows } = await pool.query('select * from events where user_email=$1 order by date asc, id asc',[email]); return rows }
  else return mem.events[email] || []
}
async function repo_addEvent(email,{ title, date }){
  if (hasPG) { const { rows } = await pool.query('insert into events(user_email,title,date) values($1,$2,$3) returning *',[email,title,date]); return rows[0] }
  else { const item={ id:String(Date.now())+Math.random().toString(36).slice(2,8), title, date, created_at:new Date().toISOString() }; mem.events[email]=(mem.events[email]||[]).concat([item]); return item }
}
async function repo_delEvent(email,id){
  if (hasPG) await pool.query('delete from events where id=$1 and user_email=$2',[id,email])
  else mem.events[email] = (mem.events[email]||[]).filter(x=>x.id!==id)
}

// ---------------- API ----------------
app.get('/api/personal', requireAuth, async (req,res)=>{
  const email = (req.session.user || req.user)?.email || 'guest@guest.local'
  const rows = await repo_listPersonal(email); res.json(rows)
})
app.post('/api/personal', requireAuth, async (req,res)=>{
  ...
  if (publish_to_shared) {
    ...
    await repo_addShared(email, { title, date, time_slot, token, share_url })
  }
  res.json({ success:true, item })
})
app.delete('/api/personal/:id', requireAuth, async (req,res)=>{
  const email = (req.session.user || req.user)?.email || 'guest@guest.local'
  await repo_delPersonal(email, req.params.id); res.json({ success:true })
})

app.get('/api/shared', requireAuth, async (req,res)=>{
  const email = (req.session.user || req.user)?.email || 'guest@guest.local'
  const rows = await repo_listShared(email); res.json(rows)
})
...
app.delete('/api/shared/:id', requireAuth, async (req,res)=>{
  const email = (req.session.user || req.user)?.email || 'guest@guest.local'
  await repo_delShared(email, req.params.id); res.json({ success:true })
})

app.get('/api/share/:token', async (req,res)=>{
  const hit = await repo_getSharedByToken(req.params.token)
...
app.get('/api/events', requireAuth, async (req,res)=>{
  const email = (req.session.user || req.user)?.email || 'guest@guest.local'
  const rows = await repo_listEvents(email); res.json(rows)
})
app.post('/api/events', requireAuth, async (req,res)=>{
  const email = (req.session.user || req.user)?.email || 'guest@guest.local'
  const { title, date } = req.body
  const item = await repo_addEvent(email, { title, date })
  res.json({ success:true, item })
})
app.delete('/api/events/:id', requireAuth, async (req,res)=>{
  const email = (req.session.user || req.user)?.email || 'guest@guest.local'
  await repo_delEvent(email, req.params.id); res.json({ success:true })
})
