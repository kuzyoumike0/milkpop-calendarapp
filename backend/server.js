...
app.post('/api/personal', requireAuth, (req,res)=>{
  const { title, date, time_slot, memo } = req.body; const user = (req.session.user||{}).email;
...
app.get('/api/personal', requireAuth, (req,res)=> { const uQ = (req.query.user||'').toString(); const uSess = (req.session.user||{}).email; const u = uQ && uQ===uSess ? uQ : uSess; const rows = u ? personalEvents.filter(e=>e.user===u) : []; res.json(rows); }); const rows = u ? personalEvents.filter(e=>e.user===u) : personalEvents; res.json(rows); });
app.delete('/api/personal/:id', requireAuth, (req,res)=>{ const { id } = req.params; personalEvents = personalEvents.filter(e=> e.id != id); res.json({ success:true }); });


app.post('/api/shared', requireAuth, (req,res)=>{
  const { title, date, time_slot } = req.body; const created_by = (req.session.user||{}).email;
  const item = { id: String(seq++), title, date, time_slot, created_by, created_at: new Date().toISOString() };
  sharedEvents.push(item);
  res.json({ success:true, item });
});
app.get('/api/shared', requireAuth, (req,res)=> { const uQ = (req.query.user||'').toString(); const uSess = (req.session.user||{}).email; const u = uQ && uQ===uSess ? uQ : uSess; const rows = sharedEvents.filter(e=>!e.created_by || e.created_by===u); res.json(rows); });
  res.json(merged);
});
...
app.delete('/api/shared/:id', requireAuth,(req,res)=>{ const {id}=req.params; sharedEvents = sharedEvents.filter(e=> e.id != id); res.json({success:true}); });
