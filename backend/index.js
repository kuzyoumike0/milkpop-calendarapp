...
app.post("/api/schedules", async (req, res) => {
  ...
  try {
    await pool.query(
      ...
      [id, title || "無題スケジュール", JSON.stringify(dates), JSON.stringify(options)]
    );
    res.json({ ok: true, id });
  } catch (err) {
...
app.post("/api/personal", async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      ...
      [newPersonalId, newShareId, title || "無題スケジュール", memo || "", JSON.stringify(dates), JSON.stringify(options)]
    );
    ...
    await pool.query(
      ...
      [newShareId, title || "無題スケジュール", JSON.stringify(dates), JSON.stringify(options)]
    );
